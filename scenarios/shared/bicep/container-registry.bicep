...
@description('Optional. The name of the Container Registry cache rule. If set, it overrides the name generated by the template.')
param cacheRuleName string = 'cache-simple-hello'

@description('Optional. Deploy the simple hello world cache rule. Default value is true.')
param deployCacheRule bool = true
...
resource registry_agentPool 'Microsoft.ContainerRegistry/registries/agentPools@2019-06-01-preview' = if (deployAgentPool) {
  parent: registry
  name: agentPoolName
  location: location
  properties: {
...
@description('This cache rule will cache the simple hello world image.')
resource registry_cacheRule 'Microsoft.ContainerRegistry/registries/cacheRules@2024-11-01-preview' = if (deployCacheRule) {
  parent: registry
  name: cacheRuleName
  properties: {
    sourceRepository: 'mcr.microsoft.com/azuredocs/containerapps-helloworld'
    targetRepository: 'azuredocs/containerapps-helloworld'
  }
}
...
output agentPoolName string = (deployAgentPool) ? registry_agentPool.name : ''
